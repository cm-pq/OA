...
<title>添加对标信息</title>
<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/kin/css/base.css">
	<%--<link type="text/css" href="${pageContext.request.contextPath}/http://code.jquery.com/ui/1.9.1/themes/smoothness/jquery-ui.css" rel="stylesheet" />
	...
	<script src="${pageContext.request.contextPath}/http://code.jquery.com/jquery-1.8.2.min.js" type="text/javascript"></script>
	<script type="text/javascript" src="${pageContext.request.contextPath}/http://code.jquery.com/ui/1.9.1/jquery-ui.min.js"></script>
...
<script type="text/javascript">
        $(function(){
           ...
           /* jQuery('.time').datetimepicker({
            ...
            $.ajax({
                type:'GET',
                url:'${pageContext.request.contextPath}/da/show',
                dataType:'json',
                success:function(resultData){


                    $(resultData).each(function(index,item){
                        var  c =new Date(item.datime);
                        var d = c.getFullYear()+"-"+c.getMonth()+"-"+c.getDay();
                          var option="<option value="+item.daid+">"+item.dacname+d+"</oiption>";
                           $("#com").append(option);
                    });
                },
            });
        ...
        function showInfo(obj){
            $.ajax({
                type:'GET',
                url:'${pageContext.request.contextPath}/da/selectone',
				data:{"cname":obj},
                dataType:'json',
                success:function(resultData){
                     console.info(resultData);
                     $("#cname1").val(resultData.datacollect.dacname);
                     $("#money1").val(resultData.datacollect.daturnover);
                     $("#year1").val(resultData.datacollect.datime);
                     $("#maindo1").val(resultData.datacollect.dabusiness);
                     $("#good1").val(resultData.datacollect.dasuperiority);
                     $("#nogood1").val(resultData.datacollect.dainforiority);
                     $("#po1").val(resultData.datacollect.dasort);
                     $("#count1").val(resultData.datacollect.empcount);
                     $("#build1").val(resultData.datacollect.buildtime);
                     $("#remark1").val(resultData.datacollect.remark);
                     $("#attachment").text(resultData.datacollect.daother);
                     // 表单数据
                     $("#pname").val(resultData.datacollect.dacname);
                     $("#comnameFk").val(resultData.datacollect.daid);
                },
            });
		}
