...
<tr align="center" bgcolor="#FAFAF1" height="22">
	...
	<c:forEach items="${baoxiao}" var="baoxiao" varStatus="i">
<tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='#FCFDEE';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22" >
	<td><input name="bx" type="checkbox" id="id" value="${i.count}" class="np"></td>
	<td>${baoxiao.bxid}</td>
  ...
  --%>
    <c:choose>
        <c:when test="${baoxiao.bxstatus==0}">
            <td>未审批</td>
            <td>
                <a href="mybaoxiao-edit.jsp?bxid=${baoxiao.bxid}">编辑</a>
                <a href="mybaoxiao-edit.jsp?bxid=${baoxiao.bxid}">查看详情</a>
                <a href="mybaoxiao-edit.jsp?bxid=${baoxiao.bxid}">撤销</a>
            </td>
        </c:when>
        <c:when test="${baoxiao.bxstatus==1}">
         <td>审批通过</td>
            <td>
                <a href="" style="pointer-events:none;color: grey" >查看详情</a>
                <a href="mybaoxiao-edit.jsp?bxid=${baoxiao.bxid}">撤销</a>
            </td>
        </c:when>
        <c:when test="${baoxiao.bxstatus==2}">
            <td>驳回</td>
            <td>
                <a href="mybaoxiao-edit-show?bxid=${baoxiao.bxid}">编辑</a>
                <a href="mybaoxiao-edit.jsp?bxid=${baoxiao.bxid}">查看详情</a>
                <a href="mybaoxiao-edit.jsp?bxid=${baoxiao.bxid}">撤销</a>
            </td>
        </c:when>
        <c:when test="${baoxiao.bxstatus==3}">
            <td>审批未付款</td>
            <td>
                <a href="javascript:void(0)" style="pointer-events:none;color: grey" >查看详情</a>
